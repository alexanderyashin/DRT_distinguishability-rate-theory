...
% ------------------------------------------------------------
% Utilities (must be before hyperref if used for link colors)
% ------------------------------------------------------------
\usepackage{xcolor}
...
% Distinguishability rate (base symbol)
% IMPORTANT: This already contains a subscript "D".
\newcommand{\DistRate}{v_{\mathrm{D}}}

% Use superscripts (not subscripts) for variants of DistRate.
% This avoids double-subscript issues like \DistRate_{\mathrm{ideal}}.
\newcommand{\DistRateIdeal}{{\DistRate}^{\mathrm{ideal}}}

% A safe "max-envelope" version of DistRate.
% Intended usage in text: \DistRateMax(t)
\newcommand{\DistRateMax}{{\DistRate}^{\max}}
