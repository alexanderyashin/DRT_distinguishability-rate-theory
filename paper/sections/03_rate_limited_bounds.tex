% ============================================================
% sections/03_rate_limited_bounds.tex  (FULL REPLACEMENT)
% ============================================================
...
All results here are strictly \emph{epistemic}: they follow solely from operational
distinguishability, fixed decision thresholds, and upper envelopes on
information-accumulation rates imposed by the observation channel.
No assumptions are made about microscopic dynamics beyond what is encoded in the
channel specification and its distinguishability-rate constraints.
...
\end{equation}
where $\FI_T(\theta)$ denotes the accumulated (classical or quantum) Fisher
information over an observation window $T$, and $\Dstar>0$ is a fixed decision
threshold.
For a given $\FI_T(\theta)$, this inequality defines the minimal
\emph{operationally resolvable} parameter scale $\delta\theta_{\min}$.

Unlike standard estimation-theoretic treatments, DRT does not assume $\FI_T$ as
given.
Instead, it asks which values of $\FI_T(\theta)$ are \emph{operationally
achievable} under finite-rate observation through a specified channel.
...
All subsequent bounds arise exclusively from constraints on
$\DistRate(t;\theta)$.
...
\item[(A3)] The observation channel admits an operationally characterizable upper
...
information, but the \emph{achievable accumulation of information itself} under
finite-rate observation.
This inversion of the usual estimation-theoretic logic is central to DRT and
should not be confused with a reformulation or tightening of the
Cramér--Rao bound.
...
channel, not intrinsic discreteness, randomness, or ill-definedness of the
parameter $\theta$.
...
Lemma~\ref{lem:dynamics_independence} is not a statement of microscopic
equivalence.
Its content is that the envelope $\DistRateMax(t)$ is an
\emph{operationally characterizable quantity}, extractable from observable
statistics without access to underlying dynamics.
Microscopic details affect DRT bounds only insofar as they modify this envelope.
...
In non-i.i.d.\ continuous monitoring, Fisher information may reside primarily in
temporal correlations rather than in marginal statistics
...
$\Flux$, $N$, or $T$.
Such deviations diagnose pre-asymptotic inference rather than violations of the
bound.
...
\item \textbf{Epistemic versus ontological regimes.}
...
within a fixed channel class indicate non-epistemic structure and are treated
separately in Section~\ref{sec:ontological_residues}.
\end{itemize}
...
envelopes invalidates the corresponding DRT description.
...
If a resolution limit persists after all reasonable
distinguishability-rate constraints are relaxed within a fixed channel class,
additional global or algebraic structure is required.
