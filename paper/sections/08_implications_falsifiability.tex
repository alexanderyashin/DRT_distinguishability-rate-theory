...
For a fixed declared observation channel, operational resolution cannot improve
faster than the envelope implied by the corresponding bound on accumulated
(distinguishability) Fisher or quantum Fisher information.
\end{proposition}
...
distinguishability-rate envelope exhibit identical operational resolution bounds,
...
Persistent and reproducible violation of any proposition above—without
modifying the declared observation channel class or introducing additional
independent distinguishability resources—falsifies DRT for the corresponding
regime.
...
\emph{Signature:} Poissonian information accumulation; inference-limited
shot-noise behavior ($\Flux^{-1/2}$).
Closure-limited self-consistent constructions (e.g.\ $\Flux^{-1/3}$) may arise
under additional estimator–window coupling assumptions.\\
...
\emph{Signature:} Information encoded in temporal correlations;
\mbox{linear-in-$T$~asymptotics}.\\
...
\item \textbf{Non-epistemic regime (channel-relative).}\\
\emph{Domain:} Constraints persisting under epistemic exhaustion within a fixed
observation channel class.\\
\emph{Signature:} Non-relaxable bounds not Fisher/QFI-reducible within that class.\\
\emph{Falsifiability:} Relaxation under increased distinguishability within the
same channel class would invalidate non-epistemic classification.
\end{itemize}
...
\item \textbf{Non-relaxable bounds} $\Rightarrow$ non-epistemic residues relative
to the declared channel class.
\end{itemize}

\subsection{Experimental programs}

\paragraph{Classical diffusion.}
Vary photon flux to test closure-limited constructions
(e.g.\ $\Flux^{-1/3}$, $\Flux^{-1/(2+\alpha)}$)
and verify collapse across systems with different diffusion constants under
identical observation models.

\paragraph{Continuous monitoring.}
Long-duration monitoring of correlated processes to test
\mbox{linear-in-$T$~}information accumulation and exclude super-linear growth
without altered correlation structure.

\paragraph{Quantum interferometry.}
Controlled tuning of visibility and decoherence in Ramsey and Mach--Zehnder
interferometry to observe meeting points and saturation consistent with
rate-envelope predictions.

\subsection{Explicit non-claims}

For clarity:
\begin{itemize}
\item No claim of fundamental discreteness of time, phase, or other parameters.
\item No prohibition of improved resolution via genuinely new resources.
\item No interpretation or modification of quantum mechanics is proposed.
\end{itemize}

\subsection{Broader implications}

DRT reframes ``limits'' as diagnostics of inference architecture.
Epistemic bounds acquire precise operational meaning, while constraints that
persist under epistemic exhaustion are isolated as non-epistemic structure
relative to fixed observation channel classes.
