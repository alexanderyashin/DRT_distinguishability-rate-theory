...
modifying the declared observation channel class or adding independent
distinguishability resources beyond that class---falsifies DRT for the
...
\emph{Signature:} Poissonian accumulation; power-law scalings (e.g.\ $\Flux^{-1/3}$).\\
...
\emph{Signature:} Information in temporal correlations; \mbox{linear-in-$T$~asymptotics}.\\
...
\emph{Falsifiability:} Unbounded growth without reducing noise or changing protocols.
...
\emph{Falsifiability:} Relaxation under increased distinguishability would invalidate ontological classification.
\end{itemize}
...
Long-duration monitoring of correlated processes to test \mbox{linear-in-$T$~}
