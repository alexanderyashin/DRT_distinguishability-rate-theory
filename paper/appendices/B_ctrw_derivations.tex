...
\begin{quote}
\textbf{Class lock.}
Equation~\eqref{eq:ctrw_closure_loop} defines a \textbf{Class~0A}
self-consistent fixed point.
It is \emph{not} a Cram\'er--Rao bound, not an optimal-observation-time
condition, and not obtained by extremizing any objective in $t$.
Any differentiation or minimization with respect to $t$ necessarily
changes the problem class.
\end{quote}
